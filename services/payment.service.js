...
const createPayment = async (data) => {
  try {
    ...
    if (!booking) {
      throw {
        err: "No booking found for the given bookng id",
        code: STATUS.BAD_REQUEST,
      };
    }
    if (booking.status === BOOKING_STATUS.successfull) {
      throw {
        err: "Booking is already successfull , cannot create booking against it",
...
const getPaymentById = async (id) => {
  try {
    ...
    if (!response) {
      throw {
        err: "No payment record found for given payment id",
